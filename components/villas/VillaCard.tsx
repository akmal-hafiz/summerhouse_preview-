...
const VillaCard = ({ property, variant = 'desktop', height }: VillaCardProps) => {
    if (variant === 'mobile') {
        ...
        return (
            <a href={externalUrl} className="flex flex-col w-[85vw] h-[600px] snap-center shrink-0 pb-27 lg:pb-0">
                ...
                <h3 className="text-[24px] text-[#1a1a19] relative bottom-[-6px] lg:bottom-[-0px] mt-6" style={{ fontFamily: 'var(--font-playfair), serif' }}>
                    {property.name}
                </h3>
                <p className="text-[13px] leading-[1.6] relative bottom-[-10px] lg:bottom-[-0px] font-light text-[#5a5651] mt-3 mb-6 line-clamp-2" style={{ fontFamily: 'var(--font-dm-sans), sans-serif' }}>
    ...
    return (
        <a href={externalUrl} target="_blank" rel="noopener noreferrer" className="flex flex-col group cursor-pointer relative">
            <div 
                className="w-full rounded-[12px] bg-[#d7cfc5] relative flex items-center justify-center overflow-hidden"
            ...
            <h3 className="text-[28px] relative bottom-[-10px] text-[#1a1a19] mt-10 group-hover:text-[#4d6a52] transition-colors duration-300" style={{ fontFamily: 'var(--font-playfair), serif' }}>
                {property.name}
            </h3>
